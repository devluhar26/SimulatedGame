...
def main():
    ...
    for name in names:
        try:
            ...
            curs_exchange.execute(f"SELECT bid_pps,time_of_execution FROM past_orders WHERE (rowid = (SELECT MAX(rowid) FROM past_orders WHERE stock= ?) AND  (stock =?))",(name,name))
